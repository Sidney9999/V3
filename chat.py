...
# --- NEU: Nur echte Angebots-Preise aus User-Text extrahieren ---
def extract_user_offer_price(text: str) -> int | None:
    """
    Liefert nur dann einen Preis zurück, wenn der User sehr wahrscheinlich ein Angebot macht.
    Beispiele, die NICHT als Angebot zählen sollen:
    - "700 sind mir zu viel" / "700 ist zu teuer"
    - "iPad Pro 13 Zoll M5 256 GB"
    Beispiele, die ALS Angebot zählen sollen:
    - "700"
    - "700€"
    - "ich biete 700"
    - "würde 700 zahlen"
    ...
    # 1) Kandidaten finden, aber Speichergrößen/Units/Modelldetails ausschließen
    candidates = []
    for m in re.finditer(r"\d{2,5}", t):
        val = int(m.group())
        after = t[m.end(): m.end() + 10]

        # Zahlen mit Einheiten = kein Preis (z.B. 256 GB, 13 Zoll, 60 Hz, etc.)
        if re.match(r"\s*(gb|tb|zoll|inch|\"|hz)\b", after):
            continue

        # typische Speicherkapazitäten nochmal hart ausschließen
    ...
    # 2) Wenn Nachricht praktisch nur aus einer Zahl besteht -> als Angebot werten (wichtig für eure Probanden!)
    if re.fullmatch(r"(?:€\s*)?\d{2,5}\s*(?:€)?", t):
        return candidates[0][0]

    # 3) Rejection/Complaint-Formulierungen -> kein Angebot
    #    (z.B. "700 sind mir zu viel" oder "700 ist zu teuer")
    rejection_patterns = [
        r"\bzu\s*teuer\b",
        r"\bzu\s*viel\b",
        r"\b(zahle|geb(e)?)\s*ich\s*nicht\b",
        r"\bwürde\s*ich\s*nicht\b",
        r"\bkann\s*ich\s*nicht\b",
    ]
    # Wenn eine dieser Phrasen in unmittelbarer Nähe zur Zahl steht, werten wir es NICHT als Angebot
    for val, s, e in candidates:
        window = t[max(0, s - 18): min(len(t), e + 18)]
        if any(re.search(p, window) for p in rejection_patterns):
            return None

    # 4) Positive Angebots-Cues (ohne die wird es NICHT als Angebot gewertet,
    #    außer es war eine reine Zahl)
    offer_cues = [
        r"\bich\s*biete\b",
        r"\bmein\s*angebot\b",
        r"\bangebot\b",
        r"\bich\s*zahle\b",
        r"\bwürde\s*\d{2,5}\s*(?:€)?\s*zahlen\b",
        r"\bfür\s*\d{2,5}\s*(?:€)?\b",
        r"\bmax(?:imal)?\b",
        r"\bhöchstens\b",
        r"\bbis\s*\d{2,5}\b",
    ...
    if any(re.search(p, t) for p in offer_cues):
        # Wenn mehrere Zahlen vorkommen, nimm die plausibelste (hier: höchste)
        return max(v for v, _, _ in candidates)

    # 5) Sonst: nicht sicher genug -> kein Angebot
    return None
...
def generate_reply(history, params: dict) -> str:
    ...
    user_price = extract_user_offer_price(last_user_msg)
...
if user_input and not st.session_state["closed"]:
    ...
    user_price = extract_user_offer_price(user_input)
